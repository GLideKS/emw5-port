...
local RM_SHIELDCOLORS = {
	-- Some players got confused why the Whirlwind Shield was green in pre-1.09 maps
	-- This should prevent that
	["DEFAULT"] = {
		[SH_PITY] = SKINCOLOR_GREEN,
		[SH_BUBBLEWRAP] = SKINCOLOR_BLUE,
		[SH_ELEMENTAL] = SKINCOLOR_GREEN,
		[SH_FORCE] = SKINCOLOR_BLUE,
		[SH_ATTRACT] = SKINCOLOR_GOLDENROD,
...
addHook("ShieldSpawn", function(player)
	...
	-- Both FDGold (SRB2TP) and Legacy Gold (FrivolousAqua's Legacy Palettes) look better (and more accurate) for the attraction shield, so use either if possible
	if orbColor then
		if orbColor == SKINCOLOR_GOLDENROD then
	...
	elseif CheckFreeslot("MT_FD_SHIELDORB") and orbType == MT_FD_SHIELDORB then
		-- TODO: Uncomment this for testing in SRB2 The Past
		-- print("\x82WARNING:\x80 Shield type "..tostring(RM_SHIELDTYPE_TO_STRING[shieldType]).." should not be used with the era "..rmShieldEra.."!")
		shieldOrb.color = RM_SHIELDCOLORS["DEFAULT"][shieldType]
	end
