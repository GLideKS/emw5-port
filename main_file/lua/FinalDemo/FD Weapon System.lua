-- Old Weapons by LJ Sonic; Ported to 2.2 by CobaltBW
...
function A_RingExplode(mo)
	if cv_oldexplosions.value then
...
local function railThinker(mo)
	...
	if mo.momx ~= 0 or mo.momy ~= 0 then
		if not P_TryMove(mo, mo.x + mo.momx, mo.y + mo.momy, true) then return true end
...
for mt = MT_BOUNCEPICKUP, MT_GRENADEPICKUP do
...
hud.add(function(v, p)
	if not G_RingSlingerGametype() or G_TagGametype() and not (p.pflags & PF_TAGIT) return end
