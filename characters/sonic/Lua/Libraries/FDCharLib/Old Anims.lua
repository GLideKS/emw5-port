...
for p in players.iterate() do
	...
	if not (p and pmo and pmo.valid and pmo.health) then continue end

	--Don't play the transform animation, directly go super
    if not (p and pmo and FDChar[pmo.skin]) then continue end
    if FDChar[pmo.skin].skiptransform then
    ...
    if FDChar[pmo.skin].nosuperspring then
